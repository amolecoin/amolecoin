...
run-client:  ## Run amolecoin with desktop client configuration. To add arguments, do 'make ARGS="--foo" run'.
	./run-client.sh ${ARGS}

run-daemon:  ## Run amolecoin with server daemon configuration. To add arguments, do 'make ARGS="--foo" run'.
	./run-daemon.sh ${ARGS}

run-help: ## Show amolecoin node help
	@go run cmd/$(COIN)/$(COIN).go --help

run-integration-test-live: ## Run the amolecoin node configured for live integration tests
	./ci-scripts/run-live-integration-test-node.sh

run-integration-test-live-disable-csrf: ## Run the amolecoin node configured for live integration tests with CSRF disabled
	./ci-scripts/run-live-integration-test-node.sh -disable-csrf

run-integration-test-live-disable-networking: ## Run the amolecoin node configured for live integration tests with networking disabled
	./ci-scripts/run-live-integration-test-node.sh -disable-networking

run-integration-test-live-cover: ## Run the amolecoin node configured for live integration tests with coverage
	./ci-scripts/run-live-integration-test-node-cover.sh

run-integration-test-live-cover-disable-csrf: ## Run the amolecoin node configured for live integration tests with CSRF disabled and with coverage
	./ci-scripts/run-live-integration-test-node-cover.sh -disable-csrf

run-integration-test-live-cover-disable-networking: ## Run the amolecoin node configured for live integration tests with networking disabled and with coverage
	./ci-scripts/run-live-integration-test-node-cover.sh -disable-networking

test: ## Run tests for Amolecoin
	@mkdir -p coverage/
...
test-386: ## Run tests for Amolecoin with GOARCH=386
	GOARCH=386 COIN=$(COIN) go test ./cmd/... -timeout=5m
	GOARCH=386 COIN=$(COIN) go test ./src/... -timeout=5m

test-amd64: ## Run tests for Amolecoin with GOARCH=amd64
...
check-newcoin: newcoin ## Check that make newcoin succeeds and no templated files are changed.
	@if [ "$(shell git diff ./cmd/amolecoin/amolecoin.go | wc -l | tr -d ' ')" != "0" ] ; then echo 'Changes detected after make newcoin' ; exit 2 ; fi
	@if [ "$(shell git diff ./cmd/amolecoin/amolecoin_test.go | wc -l | tr -d ' ')" != "0" ] ; then echo 'Changes detected after make newcoin' ; exit 2 ; fi
...
format: ## Formats the code. Must have goimports installed (use make install-linters).
	goimports -w -local github.com/amolecoin/amolecoin ./cmd
	goimports -w -local github.com/amolecoin/amolecoin ./src
...
generate: ## Generate test interface mocks and struct encoders
	...
	goimports -w -local github.com/amolecoin/amolecoin ./src/visor/mock_unspent_pooler_test.go

install-generators: ## Install tools used by go generate
	go get github.com/vektra/mockery/.../
	go get github.com/amolecoin/amoleencoder/cmd/amoleencoder
...
fuzz-base58: ## Fuzz the base58 package. Requires https://github.com/dvyukov/go-fuzz
	go-fuzz-build github.com/amolecoin/amolecoin/src/cipher/base58/internal
...
fuzz-encoder: ## Fuzz the encoder package. Requires https://github.com/dvyukov/go-fuzz
	go-fuzz-build github.com/amolecoin/amolecoin/src/cipher/encoder/internal
