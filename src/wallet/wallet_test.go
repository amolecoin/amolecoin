...
import (
	...
	"github.com/amolecoin/amolecoin/src/cipher/encrypt"
...
func TestLoadWallet(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			expect: expect{
				meta: map[string]string{
					"bip44Coin":      fmt.Sprint(bip44.CoinTypeAmolecoin),
					"coin":           string(CoinTypeAmolecoin),
					"cryptoType":     "",
					"encrypted":      "false",
...
func generateBip44Chain(t *testing.T, seed, seedPassphrase string, change uint32, num int) []*bip32.PrivateKey {
	...
	cc, err := bip44.NewCoin(ss, bip44.CoinTypeAmolecoin)
	require.NoError(t, err)
