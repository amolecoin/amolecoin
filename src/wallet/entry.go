...
// AmolecoinAddress returns the Amolecoin address of an entry. Panics if Address is not a Amolecoin address
func (we Entry) AmolecoinAddress() cipher.Address {
	return we.Address.(cipher.Address)
}

// BitcoinAddress returns the Amolecoin address of an entry. Panics if Address is not a Bitcoin address
...
func (entries Entries) getAmolecoinAddresses() []cipher.Address {
	addrs := make([]cipher.Address, len(entries))
	for i, e := range entries {
		addrs[i] = e.AmolecoinAddress()
	}
	return addrs
}
