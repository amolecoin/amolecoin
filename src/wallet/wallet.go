...
const (
	...
	// CoinTypeAmolecoin amolecoin type
	CoinTypeAmolecoin CoinType = "amolecoin"
	// CoinTypeBitcoin bitcoin type
	CoinTypeBitcoin CoinType = "bitcoin"

	// WalletTypeDeterministic deterministic wallet type.
	// Uses the original Amolecoin deterministic key generator.
...
func ResolveCoinType(s string) (CoinType, error) {
	...
	case "amole", "amolecoin":
		return CoinTypeAmolecoin, nil
	case "btc", "bitcoin":
...
type Options struct {
	...
	Coin           CoinType        // coin type: amolecoin, bitcoin, etc. Refers to which pubkey2addr method is used.
...
func newWallet(wltName string, opts Options, tf TransactionsFinder) (Wallet, error) {
	...
	case WalletTypeDeterministic, WalletTypeBip44, WalletTypeXPub:
		...
		if opts.ScanN != 0 && coin != CoinTypeAmolecoin {
			return nil, errors.New("Wallet scanning is only supported for Amolecoin address wallets")
		}
...
type Wallet interface {
	...
	GetAmolecoinAddresses() ([]cipher.Address, error)
	...
	GenerateAmolecoinAddresses(num uint64) ([]cipher.Address, error)
...
func Load(filename string) (Wallet, error) {
	...
	// Make sure "amole", "btc" normalize to "amolecoin", "bitcoin"
