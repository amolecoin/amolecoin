...
// GenerateAmolecoinAddresses is a no-op for "collection" wallets
func (w *CollectionWallet) GenerateAmolecoinAddresses(num uint64) ([]cipher.Address, error) {
	return nil, NewError(errors.New("A collection wallet does not implement GenerateAmolecoinAddresses"))
}
...
func (w *CollectionWallet) GetAmolecoinAddresses() ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeAmolecoin {
		return nil, errors.New("CollectionWallet coin type is not amolecoin")
	}
...
func (w *CollectionWallet) AddEntry(e Entry) error {
	...
	for _, entry := range w.Entries {
		if e.AmolecoinAddress() == entry.AmolecoinAddress() {
