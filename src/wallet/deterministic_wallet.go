...
// DeterministicWallet manages keys using the original Amolecoin deterministic
...
func (w *DeterministicWallet) GetAmolecoinAddresses() ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeAmolecoin {
		return nil, errors.New("DeterministicWallet coin type is not amolecoin")
	}

	return w.Entries.getAmolecoinAddresses(), nil
}
...
// GenerateAmolecoinAddresses generates Amolecoin addresses. If the wallet's coin type is not Amolecoin, returns an error
func (w *DeterministicWallet) GenerateAmolecoinAddresses(num uint64) ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeAmolecoin {
	...
	amoleAddrs := make([]cipher.Address, len(addrs))
	for i, a := range addrs {
		amoleAddrs[i] = a.(cipher.Address)
	}

	return amoleAddrs, nil
}
...
func (w *DeterministicWallet) ScanAddresses(scanN uint64, tf TransactionsFinder) error {
	...
	addrs, err := w2.GenerateAmolecoinAddresses(scanN)
	if err != nil {
		return err
	}
	...
	w2.reset()
	if _, err := w2.GenerateAmolecoinAddresses(nExistingAddrs + keepNum); err != nil {
		return err
	}
