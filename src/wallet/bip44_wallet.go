...
// Bip44Wallet manages keys using the original Amolecoin deterministic
...
// GetAmolecoinAddresses returns all Amolecoin addresses in wallet. The wallet's coin type must be Amolecoin.
func (w *Bip44Wallet) GetAmolecoinAddresses() ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeAmolecoin {
		return nil, errors.New("Bip44Wallet coin type is not amolecoin")
	}

	return append(w.ExternalEntries.getAmolecoinAddresses(), w.ChangeEntries.getAmolecoinAddresses()...), nil
}
...
// GenerateAmolecoinAddresses generates Amolecoin addresses for the external chain, and appends them to the wallet's entries array.
// If the wallet's coin type is not Amolecoin, returns an error
func (w *Bip44Wallet) GenerateAmolecoinAddresses(num uint64) ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeAmolecoin {
		return nil, errors.New("GenerateAmolecoinAddresses called for non-amolecoin wallet")
	}
	...
	return entries.getAmolecoinAddresses(), nil
}
...
func scanAddressesBip32(generateEntries func(num uint64, childIdx uint32) (Entries, error), scanN uint64, tf TransactionsFinder, initialChildIdx uint32) (Entries, error) {
	...
	addrs := entries.getAmolecoinAddresses()
