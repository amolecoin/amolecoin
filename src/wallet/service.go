...
func (serv *Service) NewAddresses(wltID string, password []byte, num uint64) ([]cipher.Address, error) {
	...
	f := func(wlt Wallet) error {
		var err error
		addrs, err = wlt.GenerateAmolecoinAddresses(num)
		return err
	}
...
func (serv *Service) ScanAddresses(wltID string, password []byte, num uint64, tf TransactionsFinder) ([]cipher.Address, error) {
	...
	f := func(wlt Wallet) error {
		...
		addrs, err = wlt.GetAmolecoinAddresses()
		if err != nil {
			return err
		}
...
// GetAmolecoinAddresses returns all addresses in given wallet
func (serv *Service) GetAmolecoinAddresses(wltID string) ([]cipher.Address, error) {
	serv.RLock()
	defer serv.RUnlock()
	if !serv.config.EnableWalletAPI {
		return nil, ErrWalletAPIDisabled
	...
	return w.GetAmolecoinAddresses()
}
