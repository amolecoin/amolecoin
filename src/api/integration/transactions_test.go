...
import (
	...
	"github.com/amolecoin/amolecoin/src/util/fee"
...
func TestLiveWalletSignTransaction(t *testing.T) {
	...
	for i, e := range w.GetEntries() {
		addrs[i] = e.AmolecoinAddress().String()
	}
	...
	txnResp, err := c.WalletCreateTransaction(api.WalletCreateTransactionRequest{
		...
		CreateTransactionRequest: api.CreateTransactionRequest{
			...
			To: []api.Receiver{
				{
					Address: w.GetEntryAt(0).AmolecoinAddress().String(),
					Coins:   totalCoinsStr,
				},
			},
		},
...
func TestStableGetTransactionV2(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			name: "genesis",
			args: []api.RequestArg{
				api.RequestArg{Key: "addrs", Value: "vczdwAwVsyVoGyZCFBoRPyWU3Qq35Ln1XB"},
			},
...
func makeLiveCreateTxnTestCases(t *testing.T, w wallet.Wallet, totalCoins, totalHours uint64) []liveCreateTxnTestCase {
	...
	return []liveCreateTxnTestCase{
...

		{
			...
			outputs: []coin.TransactionOutput{
				{
					Address: w.GetEntryAt(1).AmolecoinAddress(),
					Coins:   totalCoins - 1e3,
					Hours:   1,
				},
				{
					Address: w.GetEntryAt(0).AmolecoinAddress(),
					Coins:   1e3,
