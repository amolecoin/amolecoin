...
see [Amolecoin REST API Client Godoc](https://godoc.org/github.com/amolecoin/amolecoin/src/api#Client).
...
Args:
    ...
    bip44-coin: BIP44 coin type [optional, defaults to 8000 (amolecoin's coin type), only valid if type is "bip44"]
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/wallet/scan \
-F id=test.wlt \
-F num=10 \
-F password="${password}"
```
...
```sh
curl http://127.0.0.1:9981/api/v2/data?type=txid
```
...
```sh
curl http://127.0.0.1:9981/api/v2/data?type=txid&key=key1
```
...
```sh
curl -X POST http://127.0.0.1:9981/api/v2/data -H 'Content-Type: application/json' -d '{
    "type": "txid",
    "key": "key1",
    "val": "val1"
}'
...
```sh
curl http://127.0.0.1:9981/api/v2/transaction?page=1024&limit=2
```
