...
export class PurchaseService {
  ...
  getConfig() {
    return this.get('config').pipe(
      map((response: any) => ({
        enabled: true,
        amole_btc_exchange_rate: parseFloat(response.amole_btc_exchange_rate),
      })))
  ...
  generate(wallet: WalletBase): Observable<PurchaseOrder> {
    return this.walletsAndAddressesService.addAddressesToWallet(wallet, 1).pipe(mergeMap(address => {
      return this.post('bind', { amoleaddr: address[0].address, coin_type: 'BTC' }).pipe(
        map(response => ({
  ...
  scan(address: string) {
    return this.get('status?amoleaddr=' + address).pipe(
      map((response: any) => {
