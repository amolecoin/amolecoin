...
// Config records amolecoin node and build config
type Config struct {
	Node  NodeConfig
...
type NodeConfig struct {
	...
	// Data directory holds app data -- defaults to ~/.amolecoin
...
func (c *Config) postProcess() error {
	...
	if c.Node.WebInterfaceCert == "" {
		c.Node.WebInterfaceCert = filepath.Join(c.Node.DataDirectory, "amolecoind.cert")
	} else {
		c.Node.WebInterfaceCert = replaceHome(c.Node.WebInterfaceCert, home)
	}

	if c.Node.WebInterfaceKey == "" {
		c.Node.WebInterfaceKey = filepath.Join(c.Node.DataDirectory, "amolecoind.key")
	} else {
...
func (c *NodeConfig) RegisterFlags() {
	...
	flag.StringVar(&c.WebInterfaceCert, "web-interface-cert", c.WebInterfaceCert, "amolecoind.cert file for web interface HTTPS. If not provided, will autogenerate or use amolecoind.cert in --data-dir")
	flag.StringVar(&c.WebInterfaceKey, "web-interface-key", c.WebInterfaceKey, "amolecoind.key file for web interface HTTPS. If not provided, will autogenerate or use amolecoind.key in --data-dir")
	...
	flag.StringVar(&c.DataDirectory, "data-dir", c.DataDirectory, "directory to store app data (defaults to ~/.amolecoin)")
	flag.StringVar(&c.DBPath, "db-path", c.DBPath, "path of database file (defaults to ~/.amolecoin/data.db)")
	...
	flag.StringVar(&c.WalletDirectory, "wallet-dir", c.WalletDirectory, "location of the wallet files. Defaults to ~/.amolecoin/wallet/")
	flag.StringVar(&c.KVStorageDirectory, "storage-dir", c.KVStorageDirectory, "location of the storage data files. Defaults to ~/.amolecoin/data/")
