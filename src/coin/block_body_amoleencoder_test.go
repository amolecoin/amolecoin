...
func testAmoleencoderBlockBody(t *testing.T, obj *BlockBody) {
...
func TestAmoleencoderBlockBody(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testAmoleencoderBlockBody(t, tc.obj)
		})
	}
}
...
func testAmoleencoderBlockBodyDecodeErrors(t *testing.T, k int, tag string, obj *BlockBody) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestAmoleencoderBlockBodyDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testAmoleencoderBlockBodyDecodeErrors(t, i, "empty", emptyObj)
		testAmoleencoderBlockBodyDecodeErrors(t, i, "full", fullObj)
	}
}
