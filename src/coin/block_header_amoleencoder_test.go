...
func testAmoleencoderBlockHeader(t *testing.T, obj *BlockHeader) {
...
func TestAmoleencoderBlockHeader(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testAmoleencoderBlockHeader(t, tc.obj)
		})
	}
}
...
func testAmoleencoderBlockHeaderDecodeErrors(t *testing.T, k int, tag string, obj *BlockHeader) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestAmoleencoderBlockHeaderDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testAmoleencoderBlockHeaderDecodeErrors(t, i, "empty", emptyObj)
		testAmoleencoderBlockHeaderDecodeErrors(t, i, "full", fullObj)
	}
}
