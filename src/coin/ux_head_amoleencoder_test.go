...
func testAmoleencoderUxHead(t *testing.T, obj *UxHead) {
...
func TestAmoleencoderUxHead(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testAmoleencoderUxHead(t, tc.obj)
		})
	}
}
...
func testAmoleencoderUxHeadDecodeErrors(t *testing.T, k int, tag string, obj *UxHead) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestAmoleencoderUxHeadDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testAmoleencoderUxHeadDecodeErrors(t, i, "empty", emptyObj)
		testAmoleencoderUxHeadDecodeErrors(t, i, "full", fullObj)
	}
}
