...
func testAmoleencoderUxBody(t *testing.T, obj *UxBody) {
...
func TestAmoleencoderUxBody(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testAmoleencoderUxBody(t, tc.obj)
		})
	}
}
...
func testAmoleencoderUxBodyDecodeErrors(t *testing.T, k int, tag string, obj *UxBody) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestAmoleencoderUxBodyDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testAmoleencoderUxBodyDecodeErrors(t, i, "empty", emptyObj)
		testAmoleencoderUxBodyDecodeErrors(t, i, "full", fullObj)
	}
}
