...
func testAmoleencoderIPAddr(t *testing.T, obj *IPAddr) {
...
func TestAmoleencoderIPAddr(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testAmoleencoderIPAddr(t, tc.obj)
		})
	}
}
...
func testAmoleencoderIPAddrDecodeErrors(t *testing.T, k int, tag string, obj *IPAddr) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestAmoleencoderIPAddrDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testAmoleencoderIPAddrDecodeErrors(t, i, "empty", emptyObj)
		testAmoleencoderIPAddrDecodeErrors(t, i, "full", fullObj)
	}
}
