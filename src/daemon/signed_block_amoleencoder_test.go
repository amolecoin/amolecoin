...
func testAmoleencoderSignedBlock(t *testing.T, obj *coin.SignedBlock) {
...
func TestAmoleencoderSignedBlock(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testAmoleencoderSignedBlock(t, tc.obj)
		})
	}
}
...
func testAmoleencoderSignedBlockDecodeErrors(t *testing.T, k int, tag string, obj *coin.SignedBlock) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestAmoleencoderSignedBlockDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testAmoleencoderSignedBlockDecodeErrors(t, i, "empty", emptyObj)
		testAmoleencoderSignedBlockDecodeErrors(t, i, "full", fullObj)
	}
}
