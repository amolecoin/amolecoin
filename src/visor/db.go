...
import (
	...
	"github.com/amolecoin/amolecoin/src/util/elapse"
	"github.com/amolecoin/amolecoin/src/visor/blockdb"
	"github.com/amolecoin/amolecoin/src/visor/dbutil"
	"github.com/amolecoin/amolecoin/src/visor/historydb"
)
...
// VerifyDBAmoleencoderSafe verifies that the amoleencoder generated code has the same result as the encoder
// for all data in the blockchain
func VerifyDBAmoleencoderSafe(db *dbutil.DB, quit <-chan struct{}) error {
	return db.View("VerifyDBAmoleencoderSafe", func(tx *dbutil.Tx) error {
		return verifyDBAmoleencoderSafe(tx, quit)
	})
}

func verifyDBAmoleencoderSafe(tx *dbutil.Tx, quit <-chan struct{}) error {
	if quit == nil {
		quit = make(chan struct{})
	}

	// blockdb
	if err := blockdb.VerifyDBAmoleencoderSafe(tx, quit); err != nil {
	...
	// historydb
	if err := historydb.VerifyDBAmoleencoderSafe(tx, quit); err != nil {
