...
import (
	...
	"github.com/amolecoin/amolecoin/src/visor/dbutil"
)
...
//go:generate amoleencoder -unexported -struct Block -output-path . -package blockdb github.com/amolecoin/amolecoin/src/coin
//go:generate amoleencoder -unexported -struct UxOut -output-path . -package blockdb github.com/amolecoin/amolecoin/src/coin
//go:generate amoleencoder -unexported -struct hashPairsWrapper
//go:generate amoleencoder -unexported -struct hashesWrapper
//go:generate amoleencoder -unexported -struct sigWrapper

// hashesWrapper wraps []cipher.SHA256 so it can be used by amoleencoder
type hashesWrapper struct {
	Hashes []cipher.SHA256
}

// sigWrapper wraps cipher.Sig in struct so it can be used by amoleencoder
type sigWrapper struct {
	Sig cipher.Sig
}

// hashPairsWrapper wraps []coin.HashPair so it can be used by amoleencoder
