...
func testAmoleencoderBlock(t *testing.T, obj *coin.Block) {
...
func TestAmoleencoderBlock(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testAmoleencoderBlock(t, tc.obj)
		})
	}
}
...
func testAmoleencoderBlockDecodeErrors(t *testing.T, k int, tag string, obj *coin.Block) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestAmoleencoderBlockDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testAmoleencoderBlockDecodeErrors(t, i, "empty", emptyObj)
		testAmoleencoderBlockDecodeErrors(t, i, "full", fullObj)
	}
}
