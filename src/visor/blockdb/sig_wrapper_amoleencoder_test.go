...
func testAmoleencoderSigWrapper(t *testing.T, obj *sigWrapper) {
...
func TestAmoleencoderSigWrapper(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testAmoleencoderSigWrapper(t, tc.obj)
		})
	}
}
...
func testAmoleencoderSigWrapperDecodeErrors(t *testing.T, k int, tag string, obj *sigWrapper) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestAmoleencoderSigWrapperDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testAmoleencoderSigWrapperDecodeErrors(t, i, "empty", emptyObj)
		testAmoleencoderSigWrapperDecodeErrors(t, i, "full", fullObj)
	}
}
