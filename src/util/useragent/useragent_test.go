...
func TestDataBuild(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "with remark",
			userAgent: "Amolecoin:0.24.1(remark; foo)",
			data: Data{
				Coin:    "Amolecoin",
				Version: "0.24.1",
				Remark:  "remark; foo",
...
func TestParse(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "too long",
			userAgent: fmt.Sprintf("amolecoin:0.24.1[abcdefg](%s)", strings.Repeat("a", 245)),
			err:       ErrTooLong,
		},
		{
...
func TestDataJSON(t *testing.T) {
	...
	x, err = json.Marshal(d)
	require.NoError(t, err)
	require.Equal(t, `"amolecoin:0.25.0"`, string(x))
	...
	x, err = json.Marshal(d)
	require.NoError(t, err)
	require.Equal(t, `"amolecoin:0.25.0(foo; bar)"`, string(x))
	...
	err = json.Unmarshal([]byte(`"amolecoin:0.25.0(<>)"`), &e)
...
func TestSanitize(t *testing.T) {
	...
	x := "Amolecoin:0.25.0(foo; bar)"
	require.Equal(t, x, Sanitize(x))

	// Should not have anything stripped
	x = "Amolecoin:0.25.1(foo; bar)"
	require.Equal(t, x, Sanitize(x))
}
