// Package useragent implements methods for managing Amolecoin user agents.
//
// A amolecoin user agent has the following format:
//
//   `$NAME:$VERSION[$GIT_HASH]($REMARK)`
//
// `$NAME` and `$VERSION` are required.
//
// * `$NAME` is the coin or application's name, e.g. `Amolecoin`. It can contain the following characters: `A-Za-z0-9\-_+`.
