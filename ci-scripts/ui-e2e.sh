#!/bin/bash
# Runs UI e2e tests against a amolecoin node configured with a pinned database
...
DATA_DIR=$(mktemp -d -t amolecoin-data-dir.XXXXXX)
...
# Compile the amolecoin node
# We can't use "go run" because that creates two processes which doesn't allow us to kill it at the end
echo "compiling amolecoin"
go build -o "$BINARY" -ldflags "${GOLDFLAGS}" cmd/amolecoin/amolecoin.go

# Run amolecoin node with pinned blockchain database
echo "starting amolecoin node in background with http listener on $HOST"

./amolecoin-integration -disable-networking=true \
...
echo "amolecoin node pid=$AMOLECOIN_PID"
...
echo "shutting down amolecoin node"

# Shutdown amolecoin node
kill -s SIGINT $AMOLECOIN_PID
wait $AMOLECOIN_PID
