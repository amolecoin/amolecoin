...
# Runs "live"-mode tests against a amolecoin node that is already running
...
# The amolecoin node must be run with the wallet API enabled.
...
AMOLECOIN_INTEGRATION_TESTS=1 AMOLECOIN_INTEGRATION_TEST_MODE=$MODE RPC_ADDR=$RPC_ADDR \
	AMOLECOIN_NODE_HOST=$HOST USE_CSRF=$USE_CSRF HEADER_CHECK=$HEADER_CHECK LIVE_DISABLE_NETWORKING=$DISABLE_NETWORKING \
