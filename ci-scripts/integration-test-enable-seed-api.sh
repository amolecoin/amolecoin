#!/bin/bash
# Runs "enable-seed-api"-mode tests against a amolecoin node configured with -enable-seed-api=true
...
echo "$COIN node pid=$AMOLECOIN_PID"
...
AMOLECOIN_INTEGRATION_TESTS=1 AMOLECOIN_INTEGRATION_TEST_MODE=$MODE AMOLECOIN_NODE_HOST=$HOST WALLET_DIR=$WALLET_DIR \
...
# AMOLECOIN_INTEGRATION_TESTS=1 AMOLECOIN_INTEGRATION_TEST_MODE=$MODE RPC_ADDR=$RPC_ADDR \
