#!/bin/bash
# Runs "stable"-mode tests against a amolecoin node configured with a pinned database
...
if [[ -z $TEST || $TEST = "api" ]]; then
    ...
    AMOLECOIN_INTEGRATION_TESTS=1 AMOLECOIN_INTEGRATION_TEST_MODE=$MODE AMOLECOIN_NODE_HOST=$HOST \
    ...
    if [[ $API_FAIL -ne 0 ]]; then
        shutdown_node $COIN $AMOLECOIN_PID $BINARY
        exit $API_FAIL
    fi
