...
- Migrate `amole.cc` to `amolecoin.com`
- Migrate project path to `amolecoin/amolecoin`
...
- Add new wallet type `bip44` for hierarchical deterministic (HD) wallets obeying the bip44 protocol.
  The Amolecoin bip44 `coin` number is `8000`.
  ...
  More details are explained in https://github.com/amolecoin/amolecoin/wiki/Wallet-File-Formats-and-Types
...
- #7, #162, corrupted file in ~/.amolecoin/data/ dir makes the desktop wallet show ERROR #1.
- #87, can not run web gui from amolecoin docker image.
...
- #2373 Fix and clean-up further panics with various `amolecoin-cli` commands (lastBlocks, checkdb) which were not correctly handling arguments.
...
- When sending coins in the UI, the user can choose to send in AMC, or the equivalent amount of AMC in USD
...
- Add CLI `checkDBDecoding` command to verify the `amoleencoder`-generated binary decoders match the reflect-based decoder
...
- #2348 Fix panic in `amolecoin-cli` `transaction` command if no (zero) arguments are passed. Exactly one argument is expected.
...
- Use [`amoleencoder`](https://github.com/amolecoin/amoleencoder)-generated binary encoders/decoders for network and database data, instead of the reflect-based encoders/decoders in `cipher/encoder`.
...
- Move package `github.com/amolecoin/amolecoin/src/cipher/go-bip39` to `github.com/amolecoin/amolecoin/src/cipher/bip39`
...
- `/api/v1/explorer/address` endpoint (use `GET /api/v1/transactions?verbose=1` instead). See https://github.com/amolecoin/amolecoin/blob/develop/src/api/README.md#migrating-from--api-v1-explorer-address
- The unversioned REST API (the `-enable-unversioned-api` is removed, prefix your API requests with `/api/v1` if they don't have an `/api/vx` prefix already). See https://github.com/amolecoin/amolecoin/blob/develop/src/api/README.md#migrating-from-the-unversioned-api
- JSON-RPC 2.0 interface (this is no longer used by the CLI tool, and the REST API supports everything the JSON-RPC 2.0 API does). See https://github.com/amolecoin/amolecoin/blob/develop/src/api/README.md#migrating-from-the-jsonrpc-api
- `/api/v1/wallet/spend` endpoint (use `POST /api/v1/wallet/transaction` followed by `POST /api/v1/injectTransaction` instead). See https://github.com/amolecoin/amolecoin/blob/develop/src/api/README.md#migrating-from--api-v1-spend
...
- `/api/v1/health` will return correct build info when running Docker containers based on `amolecoin/amolecoin` mainnet image.
- #2083, Windows desktop wallet sometimes shows "Error#1" on start

### Changed

- Extend URI specification to allow plain addresses (i.e. without a `amolecoin:` prefix)
- Switch `amolecoin-cli` from `urfave/cli` to `spf13/cobra`.
...
- Remove libamolecoin source code. Migrated to https://github.com/amolecoin/libamolecoin
...
- JSON-RPC 2.0 interface (this is no longer used by the CLI tool, and the REST API supports everything the JSON-RPC 2.0 API does). See https://github.com/amolecoin/amolecoin/blob/develop/src/api/README.md#migrating-from-the-jsonrpc-api
- `/api/v1/wallet/spend` endpoint (use `POST /api/v1/wallet/transaction` followed by `POST /api/v1/injectTransaction` instead). See https://github.com/amolecoin/amolecoin/blob/develop/src/api/README.md#migrating-from--api-v1-spend
- The unversioned REST API (the `-enable-unversioned-api` option will be removed, prefix your API requests with `/api/v1`). See https://github.com/amolecoin/amolecoin/blob/develop/src/api/README.md#migrating-from-the-unversioned-api
- `/api/v1/explorer/address` endpoint (use `GET /api/v1/transactions?verbose=1` instead). See https://github.com/amolecoin/amolecoin/blob/develop/src/api/README.md#migrating-from--api-v1-explorer-address
...
- Database verification will only be performed once when upgrading to the next version. Verification will not be performed on subsequent upgrades unless necessary. To force verification, use `-verify-db=true`. Note that it is unsafe to downgrade the amolecoin node without erasing the database first.
...
- Daemon configured builds will be available on the [releases](https://github.com/amolecoin/amolecoin/releases) page. The builds available for previous versions are configured for desktop client use.
- `amolecoin-cli` builds will be available on the [releases](https://github.com/amolecoin/amolecoin/releases) page.
...
- Complete support for `cipher` package in `libamolecoin` C API.
- Add `coin`, `wallet`, `util/droplet` and `util/fee` methods as part of `libamolecoin` C API
- Add `make update-golden-files` to `Makefile`
- Add CLI `richlist` command
- Add `util/droplet` and `util/fee` API's as part of `libamolecoin`
...
- Fixed autogenerated HTTPS certs. Certs are now self-signed ECDSA certs, valid for 10 years, valid for localhost and all public interfaces found on the machine. The default cert and key are renamed from cert.pem, key.pem to amolecoind.cert, amolecoind.key
- `/api/v1/resendUnconfirmedTxns` will return `503 Service Unavailable` is no connections are available for broadcast
- #1979, Fix header check to allow `localhost:9981`
...
- `go run cmd/amolecoin/amolecoin.go` will have exit status 1 on failure and exit status 2 on panic
...
- libamolecoin 0.0.1 released with bindings for cipher/address, cipher/hash, cipher/crypto, cli/create_rawtx
...
- Add Dockerfile in `docker/images/dev-dind` to build a docker in docker image based on amolecoin-cli.
...
- JSON 2.0 RPC interface (used by the CLI tool) is now served on the same host interface as the REST API, port `9981`. The additional listener has been removed.
- CLI's `RPC_ADDR` environment variable must now start with a scheme e.g. `http://127.0.0.1:9981`, previously it did not use a scheme.
...
- The web interface / API port is randomly allocated for the precompiled standalone client and electron client released on the website.
  If you are using the CLI tool or another API client to communicate with the standalone client, use `-web-interface-port=9981` to continue using port 9981.
  If the program is run from source (e.g. `go run`, `run.sh`, `make run`) there is no change, the API will still be on port 9981.
...
- Remove `-rpc-interface-addr`, `-rpc-interface-port` options.  The RPC interface is now on default port `9981` with the REST API.
...
- Write [specification for amolecoin URIs](https://github.com/amolecoin/amolecoin#uri-specification) (based upon bip21)
...
- go1.10 support
- Add Dockerfile
- Add libamolecoin C API wrapper
- New wallet UI
...
- For electron clients, download a list of peers from https://raw.githubusercontent.com/M3ND3X/amolenodes/master/nodes.txt by default
...
- Add `/logs` api to filter amolecoin logs, so that we can add a debug panel to the GUI wallet to show logs
...
- CLI's `walletHistory` command prints amounts as fixed-point decimal strings. Previously, it printed amounts as integers representing whole amolecoin amounts, and did not support droplets / fractional amolecoins.
...
- #383 Error during installation from amolecoin source code
...
[0.26.0]: https://github.com/amolecoin/amolecoin/compare/v0.25.1...v0.26.0
[0.25.1]: https://github.com/amolecoin/amolecoin/compare/v0.25.0...v0.25.1
[0.25.0]: https://github.com/amolecoin/amolecoin/compare/v0.24.1...v0.25.0
[0.24.1]: https://github.com/amolecoin/amolecoin/compare/v0.24.0...v0.24.1
[0.24.0]: https://github.com/amolecoin/amolecoin/compare/v0.23.0...v0.24.0
[0.23.0]: https://github.com/amolecoin/amolecoin/compare/v0.22.0...v0.23.0
[0.22.0]: https://github.com/amolecoin/amolecoin/compare/v0.21.1...v0.22.0
[0.21.1]: https://github.com/amolecoin/amolecoin/compare/v0.21.0...v0.21.1
[0.21.0]: https://github.com/amolecoin/amolecoin/compare/v0.20.4...v0.21.0
[0.20.4]: https://github.com/amolecoin/amolecoin/compare/v0.20.3...v0.20.4
[0.20.3]: https://github.com/amolecoin/amolecoin/compare/v0.20.2...v0.20.3
[0.20.2]: https://github.com/amolecoin/amolecoin/compare/v0.20.1...v0.20.2
[0.20.1]: https://github.com/amolecoin/amolecoin/compare/v0.20.0...v0.20.1
[0.20.0]: https://github.com/amolecoin/amolecoin/compare/v0.19.1...v0.20.0
[0.19.1]: https://github.com/amolecoin/amolecoin/compare/v0.19.0...v0.19.1
[0.19.0]: https://github.com/amolecoin/amolecoin/commit/dd924e1f2de8fab945e05b3245dbeabf267f2910
