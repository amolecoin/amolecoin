...
The CLI command APIs can be used directly from a Go application, see [Amolecoin CLI Godoc](https://godoc.org/github.com/amolecoin/amolecoin/src/cli).
...
```bash
$ cd $GOPATH/src/github.com/amolecoin/amolecoin/cmd/amolecoin-cli
$ go install ./...
```
...
CLI will connect to amolecoin node REST API address `http://127.0.0.1:9981` by default.
You can change the address by setting the `RPC_ADDR` environment variable
with the following command:

```bash
$ export RPC_ADDR=http://127.0.0.1:9981
```
...
A username for authenticating requests to the amolecoin node.
...
A password for authenticating requests to the amolecoin node.
...
After the installation, you can run `amolecoin-cli` to see the usage:

```
$ amolecoin-cli

USAGE:
  amolecoin-cli [command] [flags] [arguments...]

DESCRIPTION:
    The amolecoin command line interface

COMMANDS:
  addPrivateKey         Add a private key to wallet
  addressBalance        Check the balance of specific addresses
  addressGen            Generate amolecoin or bitcoin addresses
  ...
  richlist              Get amolecoin richlist
  send                  Send amolecoin from a wallet or an address to a recipient address
  showConfig            Show cli configuration
  showSeed              Show wallet seed and seed passphrase
  status                Check the status of current Amolecoin node
  transaction           Show detail info of specific transaction
  verifyAddress         Verify a amolecoin address
  verifyTransaction     Verify if the specific transaction is spendable
  version               List the current version of Amolecoin components
  ...
  walletHistory         Display the transaction history of specific wallet. Requires amolecoin node rpc.
  walletKeyExport       Export a specific key from an HD wallet
  walletOutputs         Display outputs of specific wallet

FLAGS:
  -h, --help      help for amolecoin-cli
      --version   version for amolecoin-cli

Use "amolecoin-cli [command] --help" for more information about a command.

ENVIRONMENT VARIABLES:
    RPC_ADDR: Address of RPC node. Must be in scheme://host format. Default "http://127.0.0.1:9981"
    RPC_USER: Username for RPC API, if enabled in the RPC.
    RPC_PASS: Password for RPC API, if enabled in the RPC.
    COIN: Name of the coin. Default "amolecoin"
    DATA_DIR: Directory where everything is stored. Default "$HOME/.$COIN/"
```

### Add Private Key
Add a private key to a amolecoin wallet.  Wallet type must be "collection".

```bash
$ amolecoin-cli addPrivateKey [wallet] [private key]
```
...
#### Example
```bash
$ amolecoin-cli addPrivateKey $WALLET_FILE $PRIVATE_KEY
```
...
```bash
$ amolecoin-cli addressBalance [addresses]
```

#### Example
```bash
$ amolecoin-cli addressBalance 2iVtHS5ye99Km5PonsB42No3pQRGEURmxyc 2GgFvqoyk9RjwVzj8tqfcXVXB4orBwoc9qv
```
<details>
...
### Generate addresses
Generate amolecoin or bitcoin addresses.

```bash
$ amolecoin-cli addressGen [flags]
```

```
FLAGS:
  -c, --coin string    Coin type. Must be amolecoin or bitcoin. If bitcoin, secret keys are in Wallet Import Format instead of hex. (default "amolecoin")
...
#### Examples
##### Generate `n` number of amolecoin addresses
```bash
$ amolecoin-cli addressGen --num 2
```
...
```bash
$ amolecoin-cli addressGen --num 2 --coin bitcoin
```
...
```bash
$ amolecoin-cli addressGen --num 2 --hide-secrets
```
...
```bash
$ amolecoin-cli addressGen --num 2 --mode addresses
```
...
```bash
$ amolecoin-cli addressGen --num 2 --seed "my super secret seed"
```
...
```bash
amolecoin-cli addressGen --num 2 --hex
```
...
```bash
amolecoin-cli fiberAddressGen [flags]
```
...
```bash
$ amolecoin-cli addressOutputs [address list]
```

#### Example
```bash
amolecoin-cli addressOutputs tWPDM36ex9zLjJw1aPMfYTVPbYgkL2Xp9V 29fDBQuJs2MDLymJsjyWH6rDjsyv995SrGU
```
...
```bash
$ amolecoin-cli blocks [starting block or single block seq] [ending block seq]
```

#### Example
```bash
$ amolecoin-cli blocks 41 42
```
...
Checks if the given database file contains valid amolecoin blockchain data
If no argument is given, the default `data.db` in `$HOME/.$COIN/` will be checked.

```bash
$ amolecoin-cli checkdb [db path]
```

#### Example
```bash
$ amolecoin-cli checkdb $DB_PATH
```
...
```bash
$ amolecoin-cli createRawTransaction [wallet] [to address] [amount] [flags]
```
...
```bash
$ amolecoin-cli createRawTransaction $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS
```

##### Sending to a specific change address

```bash
$ amolecoin-cli createRawTransaction $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS -c $CHANGE_ADDRESS
```
...
```bash
$ amolecoin-cli createRawTransaction $WALLET_FILE -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
EOF
$ amolecoin-cli createRawTransaction $WALLET_FILE -a $FROM_ADDRESS --csv $CSV_FILE
```
...
```bash
$ amolecoin-cli createRawTransaction $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS --json
```
...
```bash
$ amolecoin-cli createRawTransactionV2 [wallet] [to address] [amount] [flags]
```
...
```bash
$ amolecoin-cli signTransaction [wallet] [raw transaction]
```

### Example

```bash
$ amolecoin-cli signTransaction $WALLET_FILE $RAW_TRANSACTION
```
...
```bash
$ amolecoin-cli decodeRawTransaction [raw transaction]
```

Decode a raw amolecoin transaction.

#### Example

```bash
amolecoin-cli decodeRawTransaction 
```
...
Encode JSON Amolecoin transaction.

```bash
$ amolecoin-cli encodeJsonTransaction [file path or -]
```
...
$ echo '  {
   ...
   }' | amolecoin-cli encodeJsonTransaction -
```
...
$ amolecoin-cli encodeJsonTransaction $FILEPATH
```
...
$ amolecoin-cli encodeJsonTransaction -f $FILEPATH
```
...
$ amolecoin-cli encodeJsonTransaction --json $FILEPATH
```
...
Broadcast a raw amolecoin transaction.
Output is the transaction id.

```bash
$ amolecoin-cli broadcastTransaction [raw transaction]
```

```bash
$ amolecoin-cli broadcastTransaction 
```
<details>
 <summary>View Output</summary>
...
### Create a wallet
Create a new Amolecoin wallet.

```bash
$ amolecoin-cli walletCreate [wallet] [flags]
```
...
FLAGS:
  ...
  -c, --coin string              Wallet address coin type (options: amolecoin, bitcoin) (default "amolecoin")
...
Creates a deterministic wallet using the [Amolecoin deterministic address generator](https://github.com/amolecoin/amolecoin/wiki/Deterministic-Keypair-Generation-Method).
Alternatively, you can create a `bip44` type wallet.

```bash
$ amolecoin-cli walletCreate $WALLET_FILE -t deterministic
```
...
```bash
$ amolecoin-cli walletCreate $WALLET_FILE -r
```
...
```bash
$ amolecoin-cli walletCreate $WALLET_FILE -m
```
...
```bash
$ amolecoin-cli walletCreate $WALLET_FILE -s "this is the super secret seed everyone needs but does not have"
```
...
```bash
$ amolecoin-cli walletCreate $WALLET_FILE -n 2
```
...
```bash
$ amolecoin-cli walletCreate $WALLET_FILE -l "cli wallet"
```
...
```bash
$ amolecoin-cli walletCreate $WALLET_FILE -t collection
```
...
```bash
$ amolecoin-cli walletCreate $WALLET_FILE -t bip44
```
...
```bash
$ amolecoin-cli walletCreate $WALLET_FILE -t xpub --xpub xpub6FHa3pjLCk84BayeJxFW2SP4XRrFd1JYnxeLeU8EqN3vDfZmbqBqaGJAyiLjTAwm6ZLRQUMv1ZACTj37sR62cfN7fe5JnJ7dh8zL4fiyLHV
```
...
Add new addresses to a amolecoin wallet.

```bash
$ amolecoin-cli walletAddAddresses [wallet] [flags]
```
...
```bash
$ amolecoin-cli walletAddAddresses $WALLET_FILE
```
...
##### Add `n` addresses
```bash
$ amolecoin-cli walletAddAddresses $WALLET_FILE -n 2
```
...
```bash
$ amolecoin-cli walletAddAddresses $WALLET_FILE --json
```
...
```bash
$ amolecoin-cli walletScanAddresses [wallet] [flags]
```
...
```bash
$ amolecoin-cli walletScanAddresses $WALLET_FILE -n 10
```
...
```bash
$ amolecoin-cli walletScanAddresses $WALLET_FILE -n 10 --json
```
...
```bash
$ amolecoin-cli walletKeyExport [wallet] [flags]
```
...
```bash
$ amolecoin-cli walletKeyExport mywallet.wlt -k xpub -p "0/0"
```
...
```bash
$ amolecoin-cli walletKeyExport mywallet.wlt -k xprv -p "0/1"
```
...
```bash
$ amolecoin-cli walletKeyExport mywallet.wlt -k pub -p "0/0/5"
```
...
```bash
$ amolecoin-cli walletKeyExport mywallet.wlt -k prv -p "0/1/5"
```
...
```bash
$ amolecoin-cli walletKeyExport mywallet.wlt -k xpub -p "2"
```
...
```bash
$ amolecoin-cli encryptWallet [wallet] [flags]
```
...
#### Encrypt wallet
```bash
$ amolecoin-cli encryptWallet $WALLET_FILE -p test
```
...
```bash
$ amolecoin-cli encryptWallet $WALLET_FILE -x sha256-xor -p test
```
...
```bash
$ amolecoin-cli decryptWallet [wallet] [flags]
```
...
### Example
```bash
$ amolecoin-cli decryptWallet $WALLET_FILE -p test
```
...
### Last blocks
Show the last `n` amolecoin blocks.
By default the last block is shown.

```bash
$  amolecoin-cli lastBlocks [numberOfBlocks]
```

#### Examples
##### Get the last block
```bash
$ amolecoin-cli lastBlocks
```
...
List addresses in a amolecoin wallet.

```bash
$ amolecoin-cli listAddresses [wallet]
```

#### Example

```bash
$ amolecoin-cli listAddresses $WALLET_FILE
```
...
### List wallets
List wallets in the Amolecoin wallet directory (`$DATA_DIR/wallets`) or in a specific directory.

```bash
$ amolecoin-cli listWallets [directory]
```
...
```json
{
    "directory": "/home/foo/.amolecoin/wallets",
    "wallets": [
        {
...
```json
{
    "directory": "/home/foo/github.com/amolecoin",
    "wallets": [
        {
            "name": "2018_02_04_45bc.wlt",
            "label": "Your Wallet",
...
```bash
$ amolecoin-cli richlist [top N addresses] [include distribution addresses]
```

#### Example
```bash
$ amolecoin-cli richlist 5 false
```
...
### Send
Make a amolecoin transaction.

```bash
$ amolecoin-cli send [wallet] [to address] [amount] [flags]
```
...
```bash
$ amolecoin-cli send $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending from a specific address in a wallet
```bash
$ amolecoin-cli send $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRRESS
```

##### Sending change to a specific change address
```bash
$ amolecoin-cli send $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS -c $CHANGE_ADDRESS
```

##### Sending to multiple addresses
```bash
$ amolecoin-cli send $WALLET_FILE -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
yExu4fryscnahAEMKa7XV4Wc1mY188KvGw,0.3
EOF
$ amolecoin-cli send $WALLET_FILE -a $FROM_ADDRESS --csv $CSV_FILE
```
...
##### Generate a JSON output
```bash
$ amolecoin-cli send $WALLET_FILE -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ amolecoin-cli showSeed [wallet] [flags]
```
...
```bash
$ amolecoin-cli showSeed $WALLET_FILE
```
...
```bash
$ amolecoin-cli showSeed $WALLET_FILE -j
```
...
```json
{
    "data_directory": "/home/user/.amolecoin",
    "coin": "amolecoin",
    "rpc_address": "http://127.0.0.1:9981"
}
```
</details>
...
```json
{
    "status": {
        ...
        "fiber": {
            "name": "amolecoin",
            "display_name": "Amolecoin",
            "ticker": "AMC",
            "coin_hours_display_name": "Coin Hours",
            "coin_hours_display_name_singular": "Coin Hour",
            "coin_hours_ticker": "SCH",
            "explorer_url": "https://explorer.amolecoin.com"
        }
    },
    "cli_config": {
        "webrpc_address": "http://127.0.0.1:9981"
    }
}
```
</details>
...
```bash
$ amolecoin-cli transaction [transaction id]
```

#### Example
```bash
$ amolecoin-cli transaction 824d421a25f81aa7565d042a54b3e1e8fdc58bed4eefe8f8a90748da6d77d135
```
...
```bash
$ amolecoin-cli addressTransactions [addr1 addr2 addr3]
```

#### Example
#### Single Address
```bash
$ amolecoin-cli addressTransactions 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
#### Multiple Address
```bash
$ amolecoin-cli addressTransactions 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda 3vbfHxPzMuyFJvgHdAoqmFnyg6k8HiLyxd
```
...
### Verify address
Verify whether a given address is a valid amolecoin addres or not.

```bash
$  amolecoin-cli verifyAddress [amolecoin address]
```
...
```bash
$ amolecoin-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
###### Invalid checksum
```bash
$ amolecoin-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsdx
```
...
```bash
$ amolecoin-cli verifyAddress 21YPg
```
...
Check the wallet a amolecoin wallet.

```bash
$ amolecoin-cli walletBalance [wallet]
```

#### Example
##### Balance of default wallet
```bash
$ amolecoin-cli walletBalance
```
...
```bash
$ amolecoin-cli walletBalance 2018_04_01_198c.wlt
```
*OR*

```bash
$ amolecoin-cli walletBalance ~/.amolecoin/wallets/2018_04_01_198c.wlt
```
...
```bash
$ amolecoin-cli walletHistory [wallet]
```

#### Example

```bash
$ amolecoin-cli walletHistory $WALLET_FILE
```
...
```bash
$ amolecoin-cli walletOutputs [wallet]
```
...
```bash
$ amolecoin-cli richlist [top N addresses (20 default)] [include distribution addresses (false default)]
```

#### Example
##### Without distribution addresses
```bash
$ amolecoin-cli richlist 2
```
<details>
 <summary>View Output</summary>
...
### CLI version
Get version of current amolecoin cli.

```bash
$ amolecoin-cli version [flags]
```
...
```bash
$ amolecoin-cli distributeGenesis [genesis address secret key]
```
...
```bash
$ amolecoin-cli distributeGenesis 87b2ae65a7475481f73288b617242261e2a451815e1c6a5478862dbb95e23e3e
```
