...
app.commandLine.appendSwitch('host-rules', 'MAP * 127.0.0.1, EXCLUDE api.coinpaprika.com, EXCLUDE swaplab.cc, EXCLUDE version.amolecoin.com, EXCLUDE downloads.amolecoin.com');
app.commandLine.appendSwitch('ssl-version-fallback-min', 'tls1.2');
app.commandLine.appendSwitch('--no-proxy-server');
app.setAsDefaultProtocolClient('amolecoin');
...
function startAmolecoin() {
  if (!dev) {
    console.log('Starting amolecoin from electron');

    if (amolecoin) {
      console.log('Amolecoin already running');
      app.emit('amolecoin-ready');
      return
    }
    ...
    // Resolve amolecoin binary location
    var appPath = app.getPath('exe');
    var exe = (() => {
      switch (process.platform) {
        case 'darwin':
          return path.join(appPath, '../../Resources/app/amolecoin');
        case 'win32':
          // Use only the relative path on windows due to short path length
          // limits
          return './resources/app/amolecoin.exe';
        case 'linux':
          return path.join(path.dirname(appPath), './resources/app/amolecoin');
        default:
          return './resources/app/amolecoin';
      }
    })()
    ...
    amolecoin = childProcess.spawn(exe, args);

    createWindow();

    amolecoin.on('error', (e) => {
      showError();
      app.quit();
    });

    amolecoin.stdout.on('data', (data) => {
      ...
      data.toString().split('\n').forEach(line => {
        if (line.indexOf(marker) !== -1) {
		  ...
		  var id = setInterval(function() {
			...
			if (splashLoaded) {
			  app.emit('amolecoin-ready', { url: currentURL });
			  clearInterval(id);
			}
		  }, 500);
        }
    ...
    amolecoin.stderr.on('data', (data) => {
      console.log(data.toString());
    });

    amolecoin.on('close', (code) => {
      // log.info('Amolecoin closed');
      console.log('Amolecoin closed');
      showError();
      reset();
    });

    amolecoin.on('exit', (code) => {
      // log.info('Amolecoin exited');
      console.log('Amolecoin exited');
      showError();
      reset();
    });
...
function createWindow(url) {
  ...
  ses.clearCache().then(response => {
    console.log('Cleared the caching of the amolecoin wallet.');
  });
    ...
    submenu: [
      {
        ...
        click: () => {
          if (walletsFolder) {
            ...
            shell.showItemInFolder(path.join(app.getPath("home"), '.amolecoin', 'wallets'));
          }
        },
      },
      {
        ...
        click: () => {
          if (walletsFolder) {
            ...
            shell.showItemInFolder(path.join(app.getPath("home"), '.amolecoin', 'logs'));
          }
        },
      },
      {
...
app.on('ready', startAmolecoin);

app.on('amolecoin-ready', (e) => {
  if (win) {
    win.loadURL(e.url);
  } else {
    createWindow(e.url);
